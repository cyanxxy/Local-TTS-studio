...
extern "C" int mlx_quantized_matmul(
  ...
  try {
    ...
    const auto& x_arr = mlx_array_get_(x);
    const auto& w_arr = mlx_array_get_(w);
    const auto& scales_arr = mlx_array_get_(scales);
    auto biases_arr = biases.ctx ? std::make_optional(mlx_array_get_(biases))
                                 : std::nullopt;
    auto group_size_opt = group_size.has_value
        ? std::make_optional<int>(group_size.value)
        : std::nullopt;
    auto bits_opt = bits.has_value ? std::make_optional<int>(bits.value)
                                   : std::nullopt;
    std::string mode_str(mode);

    // Work around a locally built MLX v0.31.2 Metal kernel issue observed on
    // Qwen3-TTS 6-bit long-sequence inputs. Full 2D/3D transposed qmm dispatch
    // can produce incorrect results with the generated mlx.metallib, while the
    // Python wheel metallib and <=16-token chunks match numerically.
    if (transpose && x_arr.ndim() == 2 && x_arr.shape(0) > 16) {
      std::vector<mlx::core::array> chunks;
      for (int start = 0; start < x_arr.shape(0); start += 16) {
        int stop = std::min(start + 16, x_arr.shape(0));
        auto chunk = mlx::core::contiguous(mlx::core::slice(
            x_arr, {start, 0}, {stop, x_arr.shape(1)}, {1, 1}, stream));
        chunks.push_back(mlx::core::quantized_matmul(
            chunk,
            w_arr,
            scales_arr,
            biases_arr,
      ...
      mlx_array_set_(*res, mlx::core::concatenate(chunks, 0, stream));
    } else if (transpose && x_arr.ndim() == 3 && x_arr.shape(1) > 16) {
      std::vector<mlx::core::array> chunks;
      for (int start = 0; start < x_arr.shape(1); start += 16) {
        int stop = std::min(start + 16, x_arr.shape(1));
        auto chunk = mlx::core::contiguous(mlx::core::slice(
            x_arr,
            {0, start, 0},
            {x_arr.shape(0), stop, x_arr.shape(2)},
            {1, 1, 1},
            stream));
        chunks.push_back(mlx::core::quantized_matmul(
            chunk,
      ...
      mlx_array_set_(*res, mlx::core::concatenate(chunks, 1, stream));
    } else {
      mlx_array_set_(
          *res,
