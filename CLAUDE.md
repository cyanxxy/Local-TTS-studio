...
Both local runtimes run on a resident Rust bridge worker (`open-tts-local-bridge --action serve-ws`, pooled by `electron/webSocketBridgeWorker.ts`). Electron launches it with `--port 0 --auth-token <token>`, waits for Rust to print `__PORT__<actual-port>`, then connects to `ws://127.0.0.1:<actual-port>/<token>`. Generation is WebSocket-only: request/progress/result metadata travels over loopback WebSocket JSON frames and audio streams as binary Float32 PCM (or Int16 PCM when chunk metadata says `encoding:"pcm16"` — used by the MLX api_server/worker relays; the Electron bridge worker converts to Float32 before the renderer). Qwen3 defaults to the upstream MLX CustomVoice 6-bit `api_server` path on macOS when configured, with `tts` CLI per-unit fallback; Candle CustomVoice keeps its model resident by repo/device/dtype/attention; Qwen3 Base clone keeps the upstream MLX worker resident by model/reference/settings. `probe` is the only one-shot subprocess action exposed to Electron. There is no interpreter discovery, adapter script, stdout generation fallback, or base64 audio payload on this path. See AGENTS.md -> "Local bridge protocol".

## Desktop Document Import

Desktop-only Import button in Studio and Reader (`document:import` IPC channel). The Electron main process opens a native file dialog (attached to the requesting window) and parses the pick with `@llamaindex/liteparse` — parsing runs on-device, no cloud API, but the first OCR of a scanned page downloads Tesseract language data once from GitHub (same first-run-download posture as model weights; failure maps to an actionable error). Logic lives in `electron/documentImport.ts`; extracted text is applied through the latest `handleTextChange` closure via a ref (an import can take minutes — a stale closure would skip the audio reset). PDFs (incl. OCR of scans) and `.txt`/`.md` work out of the box; `.docx`/`.pptx`/`.odt` need LibreOffice, images need ImageMagick, ImageMagick's PDF delegate needs Ghostscript — all fail with actionable errors. Guards: extension allowlist, 100 MB file / 800-page / 1.5M-char caps, 5-minute parse deadline (the native parse is not cancellable; the deadline only unblocks the UI). LiteParse is ESM-only and the compiled main process is CJS, so it's loaded lazily through a real dynamic `import()` (Function-constructor indirection in `main.ts` — a literal `import()` would compile to `require()`). Its native addon is unpacked from the asar via `asarUnpack` in the electron-builder config; keep that entry.
...
- The Rust bridge emits a periodic stderr heartbeat during each generation so the host inactivity watchdog isn't tripped by a slow first-run model download or long CPU inference (both are single blocking calls). Because the heartbeat keeps the host watchdog re-armed, the bridge itself enforces a child-output inactivity deadline (10 min without any MLX worker/api_server output) so a wedged child errors out instead of hanging forever. The api_server SSE stream tightens that deadline to 2 min once the first audio delta has arrived (deltas then flow at token rate). A failed api_server request evicts the resident server; if no audio was sent yet the bridge respawns and retries once before surfacing the error.
- Qwen3 warm-up (`{"command":"warm"}`) routes by `modelRepo`: MLX CustomVoice (or no repo) pre-starts the api_server; Candle CustomVoice pre-loads the in-process model but only from already-downloaded HF cache files — warm-up must never trigger a download. Base clone repos return `warmed:false`.
...
- Qwen3 MLX binaries bake in an absolute compile-time path to `mlx.metallib` (inside the build machine's cargo target dir), so `build-rust-bridge.mjs` also copies `mlx.metallib` into `dist-rust/` — MLX falls back to the metallib colocated with the executable. Without it, Qwen3 MLX works on the build machine but fails on user Macs with "Failed to load the default metallib" (verified by hiding the build dir and generating). Building the MLX binaries themselves (`npm run build:qwen3-mlx-tools`) is dev-machine-only and needs Xcode's Metal Toolchain (`xcodebuild -downloadComponent MetalToolchain`); end users only run the shipped binaries.
