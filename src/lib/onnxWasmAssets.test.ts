...
describe("onnxWasmAssets", () => {
  it("exports resolvable local ONNX runtime asset URLs", () => {
    ...
    expect(ONNX_WASM_ASSETS.jsep.mjs).toContain("ort-wasm-simd-threaded.jsep");
    expect(ONNX_WASM_ASSETS.jsep.wasm).toContain("ort-wasm-simd-threaded.jsep");
  });
  ...
  it("keeps Kokoro on the kokoro-js ONNX runtime asset version", () => {
    ...
    expect(KOKORO_ONNX_WASM_ASSETS.jsep.wasm).not.toBe(TRANSFORMERS_ONNX_WASM_ASSETS.jsep.wasm);
  });
});
