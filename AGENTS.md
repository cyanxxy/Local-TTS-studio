...
Desktop host helpers live in `electron/` (e.g. `webSocketBridgeWorker.ts` for the resident WebSocket worker pool shared by local runtimes, `qwen3MlxDownload.ts` for Qwen3 MLX model downloads, `documentImport.ts` for the desktop-only document import, `generateRateLimiter.ts`).
