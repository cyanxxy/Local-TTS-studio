...
Reader keeps the full imported book in its local library but works on one bounded, sentence-aligned reading section at a time. Chapters remain the visible table of contents; the smaller sections keep rendering, synthesis, seeking, and per-section audio caching responsive across long books. Book-relative progress, bookmarks, and selected-text notes survive section changes, while compatible cached audio resumes locally and can continue into the next section automatically. The web app persists this data in IndexedDB; Electron uses its bundled public-domain SQLite engine, and imports an existing desktop library from the previous storage format on first launch. See [local persistence](./docs/storage.md).
